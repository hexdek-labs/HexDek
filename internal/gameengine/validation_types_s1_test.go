...
// contract: judge.LossCategory* constants are intentionally
...
func TestLossCategoryValuesMatchLossCause(t *testing.T) {
	pairs := []struct {
		...
		{LossLife, judge.LossCategoryLife},
		{LossEmptyLibrary, judge.LossCategoryEmptyLibrary},
		{LossPoison, judge.LossCategoryPoison},
		{LossCommanderDamage, judge.LossCategoryCommanderDamage},
		{LossEffect, judge.LossCategoryEffect},
	}
	for _, p := range pairs {
