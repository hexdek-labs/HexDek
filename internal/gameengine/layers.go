...
func applyLayer(gs *GameState, perm *Permanent, chars *Characteristics, layer int, sublayer string) {
	...
	for _, ce := range gs.ContinuousEffects {
		...
		// CR §702.26a — a phased-out permanent's static abilities don't
		// function: skip any continuous effect generated by a phased-out
		// source. The effect stays registered and resumes when the source
		// phases back in (its dormant period is unobservable — phased-out
		// permanents are excluded from SBAs, combat, targeting, and counts).
		if ce.SourcePerm != nil && ce.SourcePerm.PhasedOut {
			continue
		}
		if ce.Layer != layer {
			continue
