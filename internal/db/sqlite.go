...
func applyMigrations(db *sql.DB) error {
	...
	migrations := []colAdd{
		...
		// measured_bracket is Freya's signal-computed bracket — the
		// canonical felt-power measurement, surfaced in the UI as the
		// "Estimated Bracket". Distinct from bracket (the declared /
		// rubber-stamp value, which may be overridden by the deck source
		// — wizards/ precons stamp to B2 regardless of measurement).
		// Default 0 = "not yet computed"; populated by the Freya
		// analyzer on next deck pass.
