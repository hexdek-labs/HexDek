...
var Archetypes = []ArchetypeDef{
	...
	{
		Name:        "Pillowfort",
		Description: "Deter attacks against you with tax effects, prevention, and punishment triggers, then win slowly",
		Keywords: []string{
			"can't attack you", "attack you unless", "attack you only if",
			"for each creature attacking you", "no more than", "can attack you",
			"prevent all damage that would be dealt to you",
			"whenever a creature deals damage to you", "skip your",
			"creatures attacking you have",
		},
		KeyCards: []string{"Ghostly Prison", "Propaganda", "Sphere of Safety",
			"No Mercy", "Solitary Confinement", "Aegis of the Gods",
			"Crawlspace", "Norn's Annex", "Windborn Muse", "Baird, Steward of Argive",
			"Archangel of Tithes", "Sun Droplet", "Story Circle"},
		Triggers: []string{"attacks", "damage"},
		Effects:  []string{"tax", "prevent_damage"},
		Weight:   4,
	},
	{
		Name:        "Group Slug",
		Description: "Passive damage and life-loss triggers that punish opponents for normal actions (casting spells, tapping lands, drawing cards)",
		Keywords: []string{
			"each opponent loses", "deals damage to each opponent",
			"whenever a player casts", "whenever an opponent casts",
			"whenever an opponent draws", "whenever an opponent taps",
			"deals damage to that player", "for each spell cast",
			"deals damage to its controller",
		},
		KeyCards: []string{"Manabarbs", "Sulfuric Vortex", "Citadel of Pain",
			"Ankh of Mishra", "Eidolon of the Great Revel", "Spellshock",
			"Pyrostatic Pillar", "Underworld Dreams", "Liliana's Caress",
			"Polluted Bonds", "Roiling Vortex", "Megrim",
			"Black Vise", "Howling Mine", "Quietus Spike",
			"Zo-Zu the Punisher", "Phyrexian Tyranny", "Smoldering Spires"},
		Triggers: []string{"cast", "upkeep"},
		Effects:  []string{"damage", "drain"},
		Weight:   4,
	},
	// ── R60 (post-precon-audit): four archetypes added after the
	// docs/precon-shape-scans/group-{a,b,c}.md pass showed Group Hug,
	// Cycling, Toxic, and Vehicles precons were falling through to
	// Midrange/Artifacts/Tribal with no specific archetype tag. ──
	{
		Name:        "Group Hug",
		Description: "Give every player cards/lands/life to deflect aggression, then pivot to an asymmetric payoff (Triskaidekaphile, Smothering Tithe, Approach of the Second Sun)",
		Keywords: []string{
			"each player draws", "each opponent draws", "target opponent draws",
			"each player gains", "each opponent gains",
			"each player may search", "each opponent may search",
			"each player may put a land",
			"each player creates",
		},
		KeyCards: []string{"Phelddagrif", "Kynaios and Tiro of Meletis",
			"Howling Mine", "Font of Mythos", "Temple Bell",
			"Heartbeat of Spring", "Veteran Explorer", "Tempting Wurm",
			"Edric, Spymaster of Trest", "Selvala, Explorer Returned",
			"Triskaidekaphile", "Smothering Tithe"},
		Triggers: []string{"upkeep", "draw"},
		Effects:  []string{"draw", "ramp", "lifegain"},
		Weight:   4,
	},
	{
		Name:        "Cycling",
		Description: "Cycle the library aggressively to fuel cycling-payoff triggers (Astral Drift, Drake Haven, New Perspectives) and assemble a graveyard-fed finish",
		Keywords: []string{
			"cycling {", "whenever you cycle", "when you cycle",
			"cards with cycling", "spells with cycling",
			"if you've cycled", "cycling abilities you activate cost",
		},
		KeyCards: []string{"Gavi, Nest Warden", "Astral Drift", "Astral Slide",
			"Drake Haven", "New Perspectives", "Fluctuator",
			"Decree of Justice", "Decree of Pain", "Valiant Rescuer",
			"Curator of Mysteries", "Living End", "Shark Typhoon"},
		Triggers: []string{"cycle"},
		Effects:  []string{"draw", "token_create", "damage"},
		Weight:   4,
	},
	{
		Name:        "Toxic",
		Description: "Win by accumulating 10 poison counters on each opponent via infect, toxic, and proliferate effects (CR 704.5c alt-win)",
		Keywords: []string{
			"infect (this creature deals damage",
		...
		KeyCards: []string{"Atraxa, Praetors' Voice", "Ixhel, Scion of Atraxa",
			"Glistener Elf", "Phyrexian Crusader", "Skithiryx, the Blight Dragon",
			"Inkmoth Nexus", "Ichor Rats", "Tainted Strike", "Phyresis",
			"Triumph of the Hordes", "Venser's Journal",
			"Vraska, Betrayal's Sting", "Karn's Bastion"},
		Triggers: []string{"damage", "etb"},
		Effects:  []string{"poison_counter", "proliferate"},
		Weight:   4,
	},
	{
		Name:        "Vehicles",
		Description: "Crew Vehicles (and Spacecraft) into the red zone with pilot-creature support; resilient bodies that ride out wraths as artifacts",
		Keywords: []string{
			"crew", "vehicle", "spacecraft",
			"whenever a vehicle", "vehicles you control",
			"crewed by", "crew costs you pay cost",
		},
		KeyCards: []string{"Smuggler's Copter", "Heart of Kiran",
			"Skysovereign, Consul Flagship", "Cultivator's Caravan",
			"Esika's Chariot", "Mobilizer Mech", "Parhelion II",
			"Kotori, Pilot Prodigy", "Inspirit, Flagship Vessel",
			"Depala, Pilot Exemplar", "Veteran Motorist",
			"Sram, Senior Edificer", "Sai, Master Thopterist"},
		Triggers: []string{"attacks", "etb"},
		Effects:  []string{"damage", "draw"},
		Weight:   4,
	},
	{
		Name:        "Damage Redirect",
		Description: "Punisher creatures that reflect damage back at opponents (Stuffy Doll / Boros Reckoner archetype), often supported by damage doublers",
		Keywords: []string{
			"is dealt damage, it deals that much damage",
			"is dealt damage, it deals",
			"would be dealt to you is dealt to",
			"redirect", "deals double", "if a source would deal damage",
		},
		KeyCards: []string{"Stuffy Doll", "Boros Reckoner", "Spitemare",
			"Truefire Captain", "Brash Taunter", "Mogg Maniac",
			"Pariah", "Pariah's Shield", "Furnace of Rath", "Gratuitous Violence",
			"Gisela, Blade of Goldnight", "Toralf, God of Fury",
			"Repercussion", "Blasphemous Act", "Curse of Bloodletting"},
		Triggers: []string{"damage"},
		Effects:  []string{"damage_redirect", "damage_double"},
		Weight:   3,
	},
}
