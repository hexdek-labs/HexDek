...
func TestBuildTierFitnessScore_BothComponentsAlignedHighFit(t *testing.T) {
	...
	got := BuildTierFitnessScore(curve, tutors, nil)
	if got == nil {
		t.Fatal("nil result")
	}
	if got.Score < 0.85 {
...
func TestBuildTierFitnessScore_NilComponentsHandled(t *testing.T) {
	// Both nil
	got := BuildTierFitnessScore(nil, nil, nil)
	if got == nil {
	...
	// Only curve
	curve := BuildManaCurveTierFit(3.2, 3)
	got = BuildTierFitnessScore(curve, nil, nil)
	if got.Score < 0.99 {
	...
	// Only tutors
	tutors := BuildTutorDensityTierFit(4, 99, 3)
	got = BuildTierFitnessScore(nil, tutors, nil)
	if got.Score < 0.99 {
