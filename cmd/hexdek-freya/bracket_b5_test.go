...
func TestEstimateBracket_FreeInteractionLifts(t *testing.T) {
	...
	bNoFree, _ := estimateBracket(noFree, report)
	bWithFree, _ := estimateBracket(withFree, report)
...
func TestEstimateBracket_B5GateRequiresCEDHMarker(t *testing.T) {
	...
	bDemoted, lblDemoted := estimateBracket(demotedCtx, report)
	if bDemoted != 4 {
	...
	bKeep, lblKeep := estimateBracket(keepCtx, report)
	if bKeep != 5 {
...
func TestEstimateBracket_B5GateRequiresLowCMC(t *testing.T) {
	...
	bSlow, lblSlow := estimateBracket(slowCtx, report)
	if bSlow != 4 {
	...
	bLean, _ := estimateBracket(leanCtx, report)
	if bLean != 5 {
