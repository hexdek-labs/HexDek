...
## The 4 ports — cheap CMC 1-2

The initial scope was 10, but the candidate pool collapses to 4
tractable picks after filtering out files already covered by prior
batches and cards whose remaining gaps are engine-deep (cast-from-
top-of-library, full `would_lose_game` / `would_take_damage`
replacement pipelines, etc.). Quality over quantity.
...
| 3 | **Kolodin, Triumph Caster** | 2 (RW) | Tribal haste static | Promote the AST-pipeline breadcrumb to a real layer-6 continuous effect granting `kw:haste` to every Mount or Vehicle the controller controls. Predicate runs at layer evaluation time so newly-entering Mounts/Vehicles pick up haste immediately. Mount/Vehicle ETB sub-triggers (saddled / artifact-creature-until-eot) kept on existing real wiring. | Avacyn anthem grant + Bello-style controller filter |
| 4 | **Arguel's Blood Fast** | 2 (1B) | DFC transform | Wire the low-life upkeep transform. The R46 implementation emitted a "transform_to_temple_of_aclazotz_engine_call_unimplemented" partial; the engine actually exposes `gameengine.TransformPermanent` (Cecil uses it for the darkness threshold), so we just call it when life ≤ 5 and `!perm.Transformed`. Idempotent on already-transformed permanents. | Cecil, Dark Knight (darkness threshold) |
