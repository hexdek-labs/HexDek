...
| 8 | Profile.jsx backend sync | **Resolved** — new `internal/preferences/` package exposes auth-gated `GET` / `PATCH /api/me/preferences` (display_name + owner_name keyed on `device_id`). Profile.jsx fetches on mount + PATCHes on save with localStorage as offline fallback. |
| 9 | r47 M1/M2/M3 janitor sweep | **Resolved** — M2 stealth-resolved earlier; M1 now uses `IsSQLitePrimaryKeyConflict` to distinguish retryable PK collisions from other DB errors; M3 `RowsAffected` swallow replaced with proper error handling |
...
| 8 | ~~`hexdek/src/screens/Profile.jsx` — Display Name / Owner Name inputs persist via `localStorage.setItem(...)` and the page explicitly tells users *"Preferences are stored locally in your browser only."*~~ | ~~The rest of the platform has a real auth model (`AuthContext`, `/api/me`, server-side device/deck ownership). The profile preferences ride on top of localStorage with no backend sync, no cross-device portability, and no schema.~~ **RESOLVED in R60.** New `internal/preferences/` package adds the `user_preferences` table (device_id PRIMARY KEY, display_name, owner_name, updated_at) plus auth-gated `GET` / `PATCH /api/me/preferences` handlers. Profile.jsx now `fetchRemotePrefs()` on mount, `patchRemotePrefs(...)` on save, falls back to localStorage when the call 401s or fails (private-mode browsers, offline, dev-mode localhost without a session). Footer copy switches between "Preferences sync to your account" and "Preferences cached locally (sign in to sync across devices)" based on the load source. |
...
- ~~**#8 Profile.jsx backend sync**~~ — **RESOLVED in R60** (see "Closed since the original audit" below).
- ~~**#9 r47 M1/M2/M3 janitor sweep**~~ — **RESOLVED in R60** (see "Closed since the original audit" below).
...
- **#9 r47 M1/M2/M3 janitor sweep** — fully closed in R60. **M1** (constraint-vs-transport errno) landed via new `internal/db/sqlite_errno.go` exposing `IsSQLitePrimaryKeyConflict` / `IsSQLiteConstraint` that introspect `modernc.org/sqlite/lib`'s extended result codes; `CreateParty`'s retry loop in `internal/db/party.go` now distinguishes PK collisions (retry with fresh code) from other constraint / transport errors (bail immediately, surface wrapped error). **M2** stealth-resolved earlier (zero `err == sql.ErrNoRows` sites in production; tree-wide lint at `internal/lint/errors_is_sql_errnorows_test.go` keeps it that way). **M3** (`SetMemberDeck` `RowsAffected` swallow) now checks the error return and surfaces it as `"set member deck: rows-affected for party=%q device=%q: %w"`. 8 regression tests in `party_r60_test.go` pin: PK conflict detection, FK violation rejection, nil/non-sqlite safety, `CreateParty` happy path + FK-bails-immediately + max-players range, `SetMemberDeck` happy path + no-row-matched error.
- **#8 Profile.jsx backend sync** — landed in R60. New `internal/preferences/` package: `user_preferences` table keyed on `device_id` (display_name + owner_name + updated_at); `EnsureSchema` / `Get` / `Patch` storage layer; auth-gated `GET` / `PATCH /api/me/preferences` HTTP handlers wrapped via `auth.Required(db, ...)` so unauthenticated requests 401. PATCH input is nullable per-field (omit / `null` to leave a field alone; non-nil empty string to clear), strings are trimmed, no-op patches don't bump `updated_at`. `cmd/hexdek-server/main.go` wires `EnsureSchema` + `Register` alongside the existing userprofile bootstrap. `hexdek/src/screens/Profile.jsx` rewritten: `fetchRemotePrefs()` on mount with localStorage fallback when the call 401s or fails; `patchRemotePrefs()` on save with localStorage mirror; new `syncStatus` state drives the footer copy between "Preferences sync to your account" (synced from server) and "Preferences cached locally (sign in to sync across devices)" (fallback path) — the old "stored locally in your browser only" line is gone. 12 regressions in `preferences_test.go`: storage layer (empty row → zero values, insert-on-first-write, partial update preserves other field, empty-string clears, whitespace trim, no-op patch doesn't bump timestamp) + HTTP handlers (GET unauth → 401, GET auth empty → zero values, PATCH unauth → 401, PATCH→GET round-trip, multi-device isolation, malformed body → 400).
