...
| 7 | Tasigur recursion + delve | **Resolved** — new `custom_tasigur_the_golden_fang.go` implements mill + opponent-pick + return; new `keywords_delve_cast.go::CastWithDelve` adds the cast-cost-reduction helper |
...
- ~~**#7 Tasigur recursion + delve**~~ — **RESOLVED in R60** (see "Closed since the original audit" below).
...
- **#7 Tasigur recursion + delve** — landed in R60 Versailles. The gen handler at `gen_tasigur_the_golden_fang.go` mill-only stub was both incomplete (no return-from-graveyard) AND duplicated the registration with the (formerly-empty) custom file at `OnActivated("Tasigur, the Golden Fang", ...)`. Fix: new `custom_tasigur_the_golden_fang.go` implements the full activated ability — pay {4}, mill 2, pick the first living opponent clockwise, have that opponent adversarially pick the LOWEST-CMC nonland card from the activator's graveyard, return it to the activator's hand. Lands are ineligible; empty/all-lands graveyards emit a no-legal-return partial. Gen file neutered (`_ = r`) to remove the duplicate. Custom is registered from `registry.go::registerDefaults`. Delve cost reduction now lives in a parallel cast helper `internal/gameengine/keywords_delve_cast.go::CastWithDelve` modeled on `CastWithConvoke` / `CastBuyback` — validates HasDelve + graveyard size, exiles `delveCount` cards via the existing `PayDelve` primitive, pays the net mana, and pushes a StackItem with `CostMeta["alt_cost"]="delve"`. New `IsDelveCast` predicate mirrors `IsConvokeCast` / `IsBuybackCast` for Heimdall/Muninn replay labeling. 11 tests in `tasigur_the_golden_fang_r60_test.go` pin: activated ability happy path + only-lands + empty-graveyard + insufficient-mana + 4-player first-living-opponent + skips-lost-opponents; `CastWithDelve` happy path + no-delve-keyword + exceeds-graveyard + exceeds-generic-cost + full-delve-zero-net. The pre-existing `era2_handlers_test.go` mill-only test updated to call the new function name and the new graveyard-after-return count. Auto-cast Hat does NOT yet choose delve counts — that's a Hat-side wiring task tracked separately; the engine primitives are now in place for it.
