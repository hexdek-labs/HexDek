...
RAW_PATTERNS = [
    ...
    # Era 1 r60 batch 3 — residual cluster sweep (after #787 landed).
    # Each pattern targets a fragment that scripts/era1_unbucketed_dump.py
    # still surfaces; ordered most-specific first.
    ("card_type_reveal_was", re.compile(r"if it was an? (?:land|creature|artifact|enchantment|instant|sorcery|planeswalker|forest|mountain|island|swamp|plains|mount|legendary) card")),
    ("card_type_reveal_milled", re.compile(r"if an? (?:land|creature|artifact|enchantment|nonland) card was (?:milled|exiled|discarded) this way|if a card with the chosen name was milled this way")),
    ("card_type_reveal_is", re.compile(r"if it'?s an? (?:forest|mountain|island|swamp|plains|mount|legendary) card|if that card is an? (?:land|creature|artifact|enchantment|legendary|instant|sorcery)|if that permanent is an? \w+ planeswalker|if a (?:creature|instant) card (?:is exiled|or a card with flash is exiled) this way")),
    ("the_exiled_or_discarded_card", re.compile(r"the exiled card (?:is|doesn'?t have)|the discarded card was an? \w+")),
    ("that_creature_pt_query", re.compile(r"that creature has (?:power|toughness) \w+ or (?:greater|more|less|fewer)|that creature'?s (?:power|toughness) is \w+ or (?:greater|more|less|fewer)|that creature has greater power or toughness")),
    ("self_pt_compare_or_was_lt", re.compile(r"its (?:power|toughness) (?:is|was) less than|toughness was less than \w+|'s power is less than (?:this creature|~'s)")),
    ("counters_fewer_than_or_exact", re.compile(r"(?:it|this creature|~) has (?:fewer than|exactly) \w+ \S+ counters? on (?:it|this)")),
    ("self_has_named_counter_bare", re.compile(r"as long as (?:~|this creature) has an? \S+ counter on (?:him|her|it)")),
    ("hand_size_exact_n", re.compile(r"you have exactly \w+ cards? in (?:your )?hand")),
    ("opp_or_player_more_cards", re.compile(r"(?:an opponent|that player) has more cards in (?:their |a |the )?hand than you")),
    ("opp_or_pw_controller_more_life", re.compile(r"planeswalker'?s controller has more life|that player .* has more life than you")),
    ("self_was_enchanted_equipped", re.compile(r"it was enchanted|it was equipped")),
    ("attached_to_creature_broad", re.compile(r"is attached to a creature|enchanted equipment is attached")),
    ("artifact_or_perm_is_creature", re.compile(r"as long as this (?:artifact|permanent) is a creature")),
    ("legendary_state_self", re.compile(r"as long as it'?s legendary|if it'?s legendary")),
    ("didnt_activate_loyalty", re.compile(r"didn'?t activate a loyalty ability")),
    ("lost_n_life_this_turn", re.compile(r"you lost (?:two|three|four|five|\d+) or more life this turn")),
    ("defending_player_poisoned", re.compile(r"defending player is poisoned")),
    ("was_dealt_damage_this_turn", re.compile(r"(?:this creature|it) was dealt damage this turn")),
    ("tribal_combat_attack", re.compile(r"a \w+ and a \w+ attacked this combat")),
    ("web_slinging", re.compile(r"web-slinging")),
    ("opp_cast_typed_spell_turn_b3", re.compile(r"an opponent cast an? \w+(?: and/or \w+)? spell this turn")),
