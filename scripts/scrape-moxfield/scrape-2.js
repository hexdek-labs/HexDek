...
const CSV_PATH = path.join(OUTPUT_DIR, 'share-links-run2.csv');
const STATE_PATH = path.join(OUTPUT_DIR, 'state-run2.json');

// Enough share-links to back 1000 deck-details with margin.
...
// List-browsing dwell, doubled — slow and deliberate.
function dwellMs() {
  const r = Math.random();
  if (r < 0.60) return randInt(3000, 12000);   // 60%   3–12s
  if (r < 0.85) return randInt(12000, 50000);  // 25%   12–50s
  if (r < 0.97) return randInt(50000, 180000); // 12%   50s–3min
  if (r < 0.995) return randInt(180000, 480000);// 2.5%  3–8min
  return randInt(480000, 1200000);             // 0.5%  8–20min
}
...
async function runSlice(page, slice, ctx) {
  ...
  try {
    ...
    log(`  goto warning: ${e.message} — continuing`);
  }
  try {
...
async function main() {
  ...
  try {
    await page.goto('https://moxfield.com/', { waitUntil: 'domcontentloaded', timeout: 60000 });
  } catch (e) {
