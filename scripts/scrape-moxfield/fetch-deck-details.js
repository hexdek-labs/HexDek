...
const SHARE_LINKS_CSV = path.join(OUTPUT_DIR, 'share-links.csv');
const DETAILS_NDJSON = path.join(OUTPUT_DIR, 'deck-details.ndjson');
const ERRORS_NDJSON = path.join(OUTPUT_DIR, 'deck-detail-errors.ndjson');
const STATE_PATH = path.join(OUTPUT_DIR, 'details-state.json');

// Optional limit for testing — set 0 to fetch all from the CSV.
const LIMIT = parseInt(process.env.LIMIT || '0', 10);

// ---------- pacing ----------
// Per-deck dwell — tightened. Phase 2 isn't browsing, it's making one
// API call per deck via the page's session. Cloudflare cares about
// fingerprint + cookies more than micro-cadence; we keep light jitter so
// it isn't a metronome but drop the heavy-tail stalls.
function dwellMs() {
  const r = Math.random();
  if (r < 0.75) return randInt(250, 1200);   // 75%  0.25–1.2s
  if (r < 0.95) return randInt(1200, 3500);  // 20%  1.2–3.5s
  if (r < 0.995) return randInt(3500, 12000);//  4.5% 3.5–12s
  return randInt(12000, 30000);              //  0.5% 12–30s
}
...
async function main() {
  ...
  log(`csv has ${publicIds.length} unique publicIds, ${publicIds.length - toFetch.length} already done, ${toFetch.length} to fetch`);
  if (toFetch.length === 0) {
    log('nothing to do.');
    return;
  }
  ...
  await page.goto('https://moxfield.com/decks/public', { waitUntil: 'networkidle2', timeout: 60000 });
  ...
  for (let i = 0; i < toFetch.length; i++) {
    ...
    if ((i + 1) % 10 === 0) {
      log(`progress: ${i + 1}/${toFetch.length}   ok=${okCount}   err=${errCount}`);
    }
  ...
  saveState(state);
  log(`done. ok=${okCount} err=${errCount}`);
